...
const Formulario = ({ enviarFormulario, alteracao }) => {
    ...
    try {
      ...
      await enviarFormulario(data);
        ...
        if (error.message.includes("Turma já existe")) {
            setError("root.serverError", { message: "Já existe uma turma com esses dados" });
        }

        if (error.message.includes("Já existe uma turma no mesmo horário")) {
            setError("root.serverError", { message: "Já existe uma turma no mesmo horário" });
        }
  ...
  return (
        <Form className="flex-fill d-flex flex-column justify-content-between mt-4" onSubmit={handleSubmit(onSubmit)}>
            <Alert variant={"danger"} className={`${errors.root ? "" : "d-none"} text-center py-1 mb-3`}>
                ...
                <Row className="gap-5">
                    ...
                    <Col>
                        ...
                        <Alert variant="white" className={`${errors.calendario_ano_letivo ? "" : "d-none"} text-danger`}>
                            ...
                            {errors?.calendario_ano_letivo?.type == "void" && "O ano letivo não existe"}
                        </Alert>
                    </Col>
                </Row>
                <Row>
