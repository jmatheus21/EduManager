    ...
    >
      <Container fluid className="d-grid gap-3">
        <Row className="gap-5">
          <Col>
            <Form.Group className="d-flex flex-column gap-1">
              ...
              <Alert
                variant="white"
                className={`${errors.ano_letivo ? "" : "d-none"} text-danger`}
              >
                {errors?.ano_letivo?.type == "equal" &&
                  "O calendário já existe"}
                {errors?.ano_letivo?.type == "required" &&
                  "O ano letivo é obrigatório"}
                {errors?.ano_letivo?.type == "validate" &&
          ...
          <Col>
            <Form.Group className="d-flex flex-column gap-1">
              <Form.Label htmlFor="diasLetivos">
                Dias letivos: <span className="text-danger">*</span>
              </Form.Label>
              <Form.Control
                id="diasLetivos"
                type="number"
              ...
              <Alert
                variant="white"
                className={`${errors.dias_letivos ? "" : "d-none"} text-danger`}
              >
                {errors?.dias_letivos?.type == "required" &&
                  "A quantidade de dias letivos é obrigatória"}
                {errors?.dias_letivos?.type == "min" &&
                  "A quantidade de dias letivos deve ser no mínimo 50"}
                {errors?.dias_letivos?.type == "max" &&
                  "A quantidade de dias letivos deve ser no máximo 200"}
              </Alert>
            </Form.Group>
          </Col>
        </Row>
        <Row className="gap-lg-5 gap-4">
          <Col>
            <Form.Group className="d-flex flex-column gap-1">
              <Form.Label htmlFor="data_inicio">
              ...
              <Form.Control
                ...
                {...register("data_inicio", {
                  required: true,
                  validate: (value) =>
                    validator.isDate(value) && anoLetivo == value.slice(0, 4),
                })}
              />
              <Alert
                variant="white"
                className={`${errors.data_inicio ? "" : "d-none"} text-danger`}
              >
                {errors?.data_inicio?.type == "required" &&
                  "A data de início é obrigatória"}
                {errors?.data_inicio?.type == "validate" &&
                  "A data de início deve ser no mesmo ano letivo inserido"}
              </Alert>
            </Form.Group>
          </Col>
          <Col>
            <Form.Group className="d-flex flex-column gap-1">
              <Form.Label htmlFor="data_fim">
              ...
              <Form.Control
                ...
                {...register("data_fim", {
                  ...
                  validate: (value) =>
                    ...
                    validator.isAfter(value, dataInicio) &&
                    anoLetivo == value.slice(0, 4),
                })}
              />
              <Alert
                variant="white"
                className={`${errors.data_fim ? "" : "d-none"} text-danger`}
              >
                {errors?.data_fim?.type == "required" &&
                  "A data de fim é obrigatória"}
                {errors?.data_fim?.type == "validate" &&
                  "A data de fim é anterior à data de início ou ela não pertence ao mesmo ano letivo"}
              </Alert>
            </Form.Group>
          </Col>
        </Row>
