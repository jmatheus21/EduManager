...
class Turma(db.Model):
    ...
    sala_numero = db.Column(db.Integer, db.ForeignKey('sala.numero', ondelete = 'SET NULL', onupdate = 'CASCADE'), nullable = True, doc="O número da sala de aula (chave estrangeira de `Sala`(numero)).")
    calendario_ano_letivo = db.Column(db.Integer, db.ForeignKey('calendario.ano_letivo', ondelete = 'CASCADE', onupdate = 'CASCADE'), nullable = False, doc="O ano letivo do calendário da turma (chave estrangeira de `Calendario`(ano_letivo)).")

    aulas = db.relationship('Aula', backref='turma', lazy=True, cascade='all, delete-orphan', doc="Relacionamento com a entidade Aula. Cada turma deve ter uma ou várias aulas associadas.")